...
if(!empty($_FILES['logo_image']['name'])){
  ...
  if(in_array($file_extension, $excepted_extension)){
    if($_FILES['logo_image']['size'] <= 1048576){
      ...
      $photo_name_query = "SELECT * FROM logos WHERE logo_color = 'dark'";
      ...
      $new_photo_name_update_query = "UPDATE logos SET logo_image = '$new_file_name' WHERE logo_color = 'dark'";
      mysqli_query($db_connect, $new_photo_name_update_query);
      // update new photo name to database end
      $_SESSION['logo_dark_edit_success'] = "Logo update successfull";
    }
    else {
      $_SESSION['logo_dark_edit_error'] = "Your file size is more than 1mb";
    }
  }
  else {
    $_SESSION['logo_dark_edit_error'] = "Your file formate is not supported";
  }  
}
else {
  $_SESSION['logo_dark_edit_error'] = "Plese select a photo";
}
